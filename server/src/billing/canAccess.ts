...
export function getUsage(db: any, tenantId: any, eventType: any, windowMs: number | null | undefined = null) {
  db = adaptDatabase(db)
  let sql =
...
export function isUnderLimit(db, tenantId, plan, metric) {
  ...
  const windowMs: number | undefined = metric === 'messages_per_day' ? 24 * 60 * 60 * 1000 : undefined
