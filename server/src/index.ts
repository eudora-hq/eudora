...
import { scopeToTenant } from './middleware/tenantScope.ts'
import { checkTrialExpiry } from './middleware/trialExpiry.ts'
...
import { loadAllJobs } from './scheduler/cronRunner.ts'
import { startApprovalMonitor } from './services/approvalGates.ts'
import { startTunnelMonitor } from './services/tunnelService.ts'
...
async function start() {
  ...
  fastify.post('/tunnel/test', async (request, reply) => {
    const { tunnelUrl } = (request.body as any) || {}
  ...
  fastify.get('/health/ollama', async (request, reply) => {
    const requestedUrl = (request.query as any)?.url || 'http://localhost:11434'
    if (
