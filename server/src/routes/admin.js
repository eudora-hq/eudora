...
export default async function adminRoutes(fastify) {
  ...
  fastify.get('/tenants', async (request, reply) => {
    ...
    const total = plan
      ? db.prepare('SELECT COUNT(*) AS count FROM tenants WHERE plan = ?').get(plan).count
      : db.prepare('SELECT COUNT(*) AS count FROM tenants').get().count
  ...
  fastify.patch('/tenants/:id/plan', async (request, reply) => {
    ...
    db.transaction(() => {
      db.prepare('UPDATE tenants SET plan = ? WHERE id = ?')
        .run(plan, request.params.id)
      seedFeatureFlags(db, request.params.id, plan)
    })()
