...
import { retrieve } from '../core/contextRetriever.ts'
import { compose } from '../core/promptComposer.ts'
...
import { createApprovalGate } from '../services/approvalGates.ts'
...
export async function executeWorkflow(
  ...
  try {
    ...
    const outputs = new Map<any, any>(results.map((result: any) => [result.nodeId, result.output || '']))
    ...
    while (queue.length > 0) {
      ...
      if (blockedByCondition || predecessors.some(edge => skipped.has(edge.source))) {
        ...
        context.currentRiskScore = Math.max(
          0,
          ...predecessors.map(edge => (riskScores.get(edge.source) as number) || 0)
        )
...
function buildGraph(nodes, edges) {
  const nodeById = new Map<any, any>(nodes.map((node: any) => [node.id, node]))
  const incoming = new Map<any, any[]>(nodes.map(node => [node.id, []]))
  const outgoing = new Map<any, any[]>(nodes.map(node => [node.id, []]))
...
async function executeNode(node, input, tenantId, db, context = {}) {
  ...
  try {
    ...
    const { content, tokensUsed, resolvedModel } = agent.model_override
      ? await relay(composed, agent.api_key_id, tenantId, agent.model_override)
      : await relay(composed, agent.api_key_id, tenantId)
